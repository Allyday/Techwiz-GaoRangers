...
<div class=" mt-6">
   <div id="table_filter" class="dataTables_filter">
      <form action="{{ route('search-order') }}" method="post" style="width:37%;float:left">
         @csrf
         @method('POST')
   ...
   <table id="tableCustomer" class="table hover row-border" style="width:100%">
      ...
      <tbody>
         ...
         <tr>
            ...
            <td class="text-center" style="width:400px">
               <form action="{{ route('update-order', $row->id) }}" method="post" style="float: left;width:70%">
                  ...
                  <a style="float: left;margin:10px" href="{{ route('order-details', $row->id) }}" class="btn btn-primary"><i class="fas fa-search-dollar"></i></a>
                  ...
                  <div class="modal-dialog" role="document">
                     <div class="modal-content">
                        ...
                        <div class="modal-body">
                           Reject order {{ $row->id }}? This action cannot be reversed
                        </div>
                        <div class="modal-footer">
                           <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                           <form action="{{ route('reject-order', $row->id) }}" method="post">
                              @csrf
                              @method('POST')
         ...
         @endif
         @if ($row->orderStatus >2 && $row->orderStatus < 8) <tr>
            <td>{{ $row->id }}</td>
            <td>{{ $row->firstName }} {{ $row->lastName }}</td>
            <td>@php
               echo getStatusOrderTag($row->orderStatus);
            ...
            <td class="text-center">
               <form action="{{ route('update-order', $row->id) }}" method="post">
                  ...
                  @method('POST')
                  <input type="hidden" id="orderStatus" name="orderStatus" value="{{ $status }}">
                  <a href="{{ route('order-details', $row->id) }}" class="btn btn-primary"><i class="fas fa-search-dollar"></i></a>
                  <button type="submit" class="btn btn-success fw-bold" onclick="setTimeout(status({{ $status }}), 5000)" id="btn-update"><i class="fa fa-angle-double-right"></i></button>
               </form>
